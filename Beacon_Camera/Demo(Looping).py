...
def subscribe(topic):
    client = paho.Client("OCR_Client_Subscriber",clean_session=True,)
    ...
    counter = 0
    #while loop_flag == 1 and counter <=1000000:
    while loop_flag == 1:
...
def process():
    ...
    if (imgready == True):
        ...
        publisher = text_publisher(pub_broker, pub_port, published_topic, payload)
        publisher.publish()
        ...
        data = {}
        topics = []
        print('Processing finished! Now starting a fresh loop')
        #client.disconnect()
        #client.loop_stop()
        time.sleep(30)
        return complete
